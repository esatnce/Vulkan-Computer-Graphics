...
// 
// Cubemap Texture:
// Collections of siz images corresponds to siz sides of cube
// indexed by a direction vector. 
// 
// UV Intervals:
// In some cases UV intervals can fall out of [0 1]
// Several Behaviours are possible: Clamp, Repeat, Mirror, Constant
//  Clamp   : Extends the colors of the borders to the texels that are outside the [0 1]
//  Repeat  : Considers only fractional part, discards the integer part.
//  Mirror  : Repeats the pattern of the texture but flips at every iteration
//  Constant: Replaces outside of the [0 1] with the constant object color
// By combining different techniques, different effects can be achieved. 
// 
// Texture Filtering:
// UV coordinates are floating point numbers while TEXELS ARE INDEXED BY INTEGER VALUES. 
// Shape of a pixel in the screen might correspond to several texels on the texture. -> Such problems are solved by filtering
// 
// If texel > pixel : Magnification Filtering Problem 
// If pixel > texel : Minification Filtering Problem
// 
// Two Magnification Filters: Nearest Pixel, Bilinear Interpolation
// Nearest Pixel: Lookup procedure first transformas UV coordinates wrt texture size, then returns texel that only considers the integer part. 
//                Produces blocky images. 
// (Bi)Linear Interpolation: Interpolates the color of the pixel from the nearest neighbors. 
//                           Produces smooth images. 
// 
// Minification Filters: Nearest Pixel, Bilinear Interpolation, Mip-mapping, Trilinear Interpolation, Rip Mapping, Anisotropic
// 
// MIPMAPPING: Easiest solution fot minification. Precomputes a set of scaled versions of the texture (called levels), each one is halved 
// size, and fetches texel from the one whose size is closest to the pixel on the screen. Requires 33% extra space. 
// 
// Trilinear Interpolation: Computes required pixel two times using the two closest levels of mipmap. Then interpolates. Smooth transition
//                          between mipmaps. 
// 
// Rip-Mapping: If surfaces are angled, mimaps tends to produce blurred images. Rip map also computes rectangular scaling scaling of original
//              texture. 4x memory of a standard texture. 
//              If surfaces are angled wrt to border of the screen shape is not a square or rectangle, its a trapezoid. Rip mapping cannot handle. 
// 
// Anisotropic Unconstrained Filter: Tries to approximate exact area of the texels that corresponds to a pixel on the screen. 
//                                   Although very effective, might slow down rendering. 
// 
// TEXTURES IN SHADERS 
// Textures are passed to shaders as particular uniform variables of Combined Texture Sample
// Sampler1D, Sampler2D, Sampler3D, SamplerCube
// Shader can obtain the color at a given position with the texture command
// vec4 outcolor = (vec3 color, 1.0 )    Last component is transperancy!
// 
// UV TRANSFORMING
// UV coordinates can be transformed before applying to surface. Allows scaling or rotating a texture before applying. 
// 
// 
...
const std::vector<Model> SceneToLoad = {
	{"Sphere.obj", OBJ, "Diffuse.png", "Specular.png", {1.0 ,0.65, -0.5}, 1.0},
	{"LargePlane.obj", OBJ, "Diffuse.png", "Specular.png", {0, -1.5, 0.0}, 4.0},
	{"Lego Friends Baker 2.obj", OBJ, "AllEffectDiff.png", "AllEffectSpecular.png", {0,0.0, 0.0}, 0.2},
	{"Lego Friends Baker Floor.obj", OBJ, "AllEffectFloor.png", "AllEffectFloorSpecular.png", {0,0.0, 0.0}, 0.2}
};
...
std::vector<SingleText> SceneText = {
	{3, {"1: Material Sphere","Lambert + Phong","Spot Light",""}, 0, 0},
	{3, {"2: Lego Friends","Ambient + Blinn","Direct Light",""}, 0, 0}};
...
private:
	...
	// Phong pipeline
 	VkDescriptorSetLayout PhongDescriptorSetLayout1;
 	VkDescriptorSetLayout PhongDescriptorSetLayout2;
  	VkPipelineLayout PhongPipelineLayout1;
  	VkPipelineLayout PhongPipelineLayout2;
	VkPipeline PhongPipeline1;
	VkPipeline PhongPipeline2;
	//// For the first uniform (per object)
	std::vector<VkBuffer> uniformBuffers;
	std::vector<VkDeviceMemory> uniformBuffersMemory;
	//// For the second uniform (per scene)
	std::vector<VkBuffer> globalUniformBuffers1;
	std::vector<VkDeviceMemory> globalUniformBuffersMemory1;	
	std::vector<VkBuffer> globalUniformBuffers2;
	std::vector<VkDeviceMemory> globalUniformBuffersMemory2;	
	// to access uniforms in the pipeline
	std::vector<VkDescriptorSet> PhongDescriptorSets1;
	std::vector<VkDescriptorSet> PhongDescriptorSets2;
	...
	// Camera
	glm::vec3 CamAng = glm::vec3(0.0f, -0.7f, 0.0f);
	glm::vec3 CamPos = glm::vec3(-2.0f, 0.5f, 3.0f);
	...
	static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
		auto app = reinterpret_cast<Assignment20*>
...
//			VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT | // disabled in the tutorial
	...
	void createDescriptorSetLayouts() {
		createPhongDescriptorSetLayout1();
		createPhongDescriptorSetLayout2();
		createTextDescriptorSetLayout();
	}
	
	void createPhongDescriptorSetLayout1() {
		...
		samplerLayoutBindingS.binding = 3;
		samplerLayoutBindingS.descriptorType =
						VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		samplerLayoutBindingS.descriptorCount = 1;
		...
		VkResult result = vkCreateDescriptorSetLayout(device, &layoutInfo,
									nullptr, &PhongDescriptorSetLayout1);
		if (result != VK_SUCCESS) {
			PrintVkError(result);
	...
	void createPhongDescriptorSetLayout2() {
		VkDescriptorSetLayoutBinding uboLayoutBinding{};
		uboLayoutBinding.binding = 0;
		uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
		uboLayoutBinding.descriptorCount = 1;
		...
		VkResult result = vkCreateDescriptorSetLayout(device, &layoutInfo,
									nullptr, &PhongDescriptorSetLayout2);
		if (result != VK_SUCCESS) {
			PrintVkError(result);
			throw std::runtime_error("failed to create descriptor set layout!");
		}
	...
	void createTextDescriptorSetLayout() {
 	...
 	void createPhongPipeline() {
 		createPipeline("BRDF1Vert.spv", "BRDF1Frag.spv",
 					    PhongPipelineLayout1, PhongPipeline1,
 					    PhongDescriptorSetLayout1, VK_COMPARE_OP_LESS, VK_POLYGON_MODE_FILL,
 					    VK_CULL_MODE_NONE, false, phongAndSkyBoxVertices);
 		createPipeline("BRDF2Vert.spv", "BRDF2Frag.spv",
 					    PhongPipelineLayout2, PhongPipeline2,
 					    PhongDescriptorSetLayout2, VK_COMPARE_OP_LESS, VK_POLYGON_MODE_FILL,
 					    VK_CULL_MODE_NONE, false, phongAndSkyBoxVertices);
 	}
	...
	void loadModelWithTexture(const Model& M, int i) {
		...
		createTextureImage(M.TextureFileS, Scene[i].TDS);
		createTextureImageView(Scene[i].TDS);
		createTextureSampler(Scene[i].TDS);
	}
...
//std::cout << k << " " << j << " " << i << " " << vb << " " << ib << " " << c << "\n";
	...
	void createUniformBuffers() {
		...
		bufferSize = sizeof(GlobalUniformBufferObject1);

		globalUniformBuffers1.resize(swapChainImages.size());
		globalUniformBuffersMemory1.resize(swapChainImages.size());

		for (size_t i = 0; i < swapChainImages.size(); i++) {
			createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
								 	 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
								 	 VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
								 	 globalUniformBuffers1[i], globalUniformBuffersMemory1[i]);
		}

		bufferSize = sizeof(GlobalUniformBufferObject2);

		globalUniformBuffers2.resize(swapChainImages.size());
		globalUniformBuffersMemory2.resize(swapChainImages.size());

		for (size_t i = 0; i < swapChainImages.size(); i++) {
			createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
								 	 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
								 	 VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
								 	 globalUniformBuffers2[i], globalUniformBuffersMemory2[i]);
		}
	...
	void createDescriptorPool() {
		...
		poolSizes[0].descriptorCount = static_cast<uint32_t>(swapChainImages.size() * Scene.size() * 8);
		poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		poolSizes[1].descriptorCount = static_cast<uint32_t>(swapChainImages.size() * Scene.size() * 8);
		poolSizes[2].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
		poolSizes[2].descriptorCount = static_cast<uint32_t>(swapChainImages.size() * Scene.size() * 4);
		...
		poolInfo.maxSets = static_cast<uint32_t>(swapChainImages.size() * (Scene.size() * 2 + 2));
	...
	void createDescriptorSets() {
		createPhongDescriptorSets1();
		createPhongDescriptorSets2();
		createTextDescriptorSets();
	}
	
	void createPhongDescriptorSets2() {
		std::vector<VkDescriptorSetLayout> layouts(swapChainImages.size() * Scene.size(),
												   PhongDescriptorSetLayout2);
		...
		PhongDescriptorSets2.resize(swapChainImages.size() * Scene.size());
		
		VkResult result = vkAllocateDescriptorSets(device, &allocInfo,
											PhongDescriptorSets2.data());
		if (result != VK_SUCCESS) {
			PrintVkError(result);
		...
		for (size_t k = 0; k < swapChainImages.size(); k++) {
			for (size_t j = 0; j < Scene.size(); j++) {
				...
				globalBufferInfo.buffer = globalUniformBuffers2[k];
				globalBufferInfo.offset = 0;
				globalBufferInfo.range = sizeof(GlobalUniformBufferObject2);
				
				VkDescriptorImageInfo imageInfoS{};
				imageInfoS.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
				imageInfoS.imageView = Scene[j].TDS.textureImageView;
				imageInfoS.sampler = Scene[j].TDS.textureSampler;
				
				std::array<VkWriteDescriptorSet, 4> descriptorWrites{};
				descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				descriptorWrites[0].dstSet = PhongDescriptorSets2[i];
				...
				descriptorWrites[1].dstSet = PhongDescriptorSets2[i];
				...
				descriptorWrites[2].dstSet = PhongDescriptorSets2[i];
				...
				descriptorWrites[3].dstSet = PhongDescriptorSets2[i];
				descriptorWrites[3].dstBinding = 3;
				descriptorWrites[3].dstArrayElement = 0;
				descriptorWrites[3].descriptorType =
											VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
				descriptorWrites[3].descriptorCount = 1;
				descriptorWrites[3].pImageInfo = &imageInfoS;
	...
	void createPhongDescriptorSets1() {
		std::vector<VkDescriptorSetLayout> layouts(swapChainImages.size() * Scene.size(),
												   PhongDescriptorSetLayout1);
		VkDescriptorSetAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
		allocInfo.descriptorPool = descriptorPool;
		allocInfo.descriptorSetCount = static_cast<uint32_t>(swapChainImages.size() * Scene.size());
		...
		PhongDescriptorSets1.resize(swapChainImages.size() * Scene.size());
		
		VkResult result = vkAllocateDescriptorSets(device, &allocInfo,
											PhongDescriptorSets1.data());
		if (result != VK_SUCCESS) {
			PrintVkError(result);
			throw std::runtime_error("failed to allocate descriptor sets!");
		}
		...
		for (size_t k = 0; k < swapChainImages.size(); k++) {
			for (size_t j = 0; j < Scene.size(); j++) {
				...
				VkDescriptorBufferInfo globalBufferInfo{};
				globalBufferInfo.buffer = globalUniformBuffers1[k];
				globalBufferInfo.offset = 0;
				globalBufferInfo.range = sizeof(GlobalUniformBufferObject1);
				...
				std::array<VkWriteDescriptorSet, 4> descriptorWrites{};
				descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				descriptorWrites[0].dstSet = PhongDescriptorSets1[i];
				descriptorWrites[0].dstBinding = 0;
				descriptorWrites[0].dstArrayElement = 0;
				descriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
				descriptorWrites[0].descriptorCount =  1;
				...
				descriptorWrites[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				descriptorWrites[1].dstSet = PhongDescriptorSets1[i];
				descriptorWrites[1].dstBinding = 1;
				descriptorWrites[1].dstArrayElement = 0;
				descriptorWrites[1].descriptorType =
											VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
				...
				descriptorWrites[2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				descriptorWrites[2].dstSet = PhongDescriptorSets1[i];
				descriptorWrites[2].dstBinding = 2;
				descriptorWrites[2].dstArrayElement = 0;
				descriptorWrites[2].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
				descriptorWrites[2].descriptorCount =  1;
				...
				descriptorWrites[3].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				descriptorWrites[3].dstSet = PhongDescriptorSets1[i];
				descriptorWrites[3].dstBinding = 3;
				descriptorWrites[3].dstArrayElement = 0;
				descriptorWrites[3].descriptorType =
											VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    ...
    void createCommandBuffers() {
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			vkCmdBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
					curText == 0 ? PhongPipeline1 : PhongPipeline2);					
			for(int j = 2*curText; j < 2*(curText+1); j++) {
//			{int j = curText;
					...
					vkCmdBindDescriptorSets(commandBuffers[i],
									...
									curText == 0 ? PhongPipelineLayout1 : PhongPipelineLayout2, 0, 1,
									curText == 0 ? 
										&PhongDescriptorSets1[i + j * swapChainImages.size()] :
										&PhongDescriptorSets2[i + j * swapChainImages.size()],
									0, nullptr);
...
//for(int ti=0;ti<4;ti++){for(int tj=0;tj<4;tj++){std::cout << tM[ti][tj] << " ";}std::cout<<"\n";}std::cout<<"\n";
		...
		if(curText == 0) {
			...
			gubo1.lightColor = glm::vec3(0.9f, 0.9f, 0.9f);
			gubo1.lightDir = glm::vec3(cos(glm::radians(135.0f)) * cos(glm::radians(-30.0f)), sin(glm::radians(135.0f)), 	cos(glm::radians(135.0f)) * sin(glm::radians(-30.0f)));
			gubo1.lightPos = glm::vec3(-3.0, 3.0, 1.0);
			gubo1.lightParams = glm::vec4(
					cos(glm::radians(22.5f)), cos(glm::radians(30.0f)), 1.0, 1.8f
				);
			gubo1.eyePos = CamPos;

			void* data;
			vkMapMemory(device, globalUniformBuffersMemory1[currentImage], 0,
								sizeof(gubo1), 0, &data);
			memcpy(data, &gubo1, sizeof(gubo1));
			vkUnmapMemory(device, globalUniformBuffersMemory1[currentImage]);
		} else {		
			GlobalUniformBufferObject2 gubo2{};
			gubo2.lightColor = glm::vec3(0.9f, 0.9f, 0.9f);
			gubo2.lightDir = glm::vec3(-0.4830f, 0.8365f, -0.2588f);
			gubo2.eyePos = CamPos;

			void* data;
			vkMapMemory(device, globalUniformBuffersMemory2[currentImage], 0,
								sizeof(gubo2), 0, &data);
			memcpy(data, &gubo2, sizeof(gubo2));
			vkUnmapMemory(device, globalUniformBuffersMemory2[currentImage]);
		}
	...
	void cleanupSwapChain() {
		...
		vkDestroyPipeline(device, PhongPipeline1, nullptr);
		vkDestroyPipelineLayout(device, PhongPipelineLayout1, nullptr);
		vkDestroyPipeline(device, PhongPipeline2, nullptr);
		vkDestroyPipelineLayout(device, PhongPipelineLayout2, nullptr);
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			vkDestroyBuffer(device, globalUniformBuffers1[i], nullptr);
			vkFreeMemory(device, globalUniformBuffersMemory1[i], nullptr);
			vkDestroyBuffer(device, globalUniformBuffers2[i], nullptr);
			vkFreeMemory(device, globalUniformBuffersMemory2[i], nullptr);
		}		
		for (size_t i = 0; i < swapChainImages.size(); i++) {
    ...
    void cleanup() {
    	...
    	for (size_t i = 0; i < Scene.size(); i++) {
	    	...
	    	vkDestroySampler(device, Scene[i].TDS.textureSampler, nullptr);
	    	vkDestroyImageView(device, Scene[i].TDS.textureImageView, nullptr);
			vkDestroyImage(device, Scene[i].TDS.textureImage, nullptr);
			vkFreeMemory(device, Scene[i].TDS.textureImageMemory, nullptr);
    	...
    	vkDestroyDescriptorSetLayout(device, PhongDescriptorSetLayout1, nullptr);
    	vkDestroyDescriptorSetLayout(device, PhongDescriptorSetLayout2, nullptr);
